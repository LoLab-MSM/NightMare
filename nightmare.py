...
import pymc
import pylab as plt
from pysb.hessian import cost_function

class Nightmare():
    def __init__(self,model,cost_function,time,start_parameters,savename):
        self.model = model
    ...
    def run_DREAM(self):
        model = pymc.Model()
        with model:
            params = pymc.Normal('params', mu=self.start_parameters, sd=np.array([1.0]*len(self.start_parameters)), shape=(len(self.start_parameters)))
            ...
            step = pymc.Dream(variables=[params], nseedchains=nseedchains, blocked=True, start_random=False,
                               save_history=True, parallel=True, adapt_crossover=False,verbose=False,
                               model_name = self.savename)

            

            trace = pymc.sample(10000, step,start=self.pso_results, njobs=self.nchains,
                                use_mpi=False,
                                progressbar=False) 
